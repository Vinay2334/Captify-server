...
class FasterWhisperASR(ASRBase):
    ...
    def load_model(self, modelsize=None, cache_dir=None, model_dir=None):
        ...
        if model_dir is not None:
            logger.debug(
                f"Loading whisper model from model_dir {
                    model_dir}. modelsize and cache_dir parameters are not used."
            )
    ...
    def transcribe(self, audio, init_prompt=""):
        ...
        sf.write(buffer, audio, samplerate=44100,
                 format="WAV", subtype="FLOAT")
        buffer.seek(0)  # Reset buffer's position to the beginning
        print("waveBuff", buffer)
...
class MLXWhisper(ASRBase):
    ...
    def load_model(self, modelsize=None, cache_dir=None, model_dir=None):
        ...
        if model_dir is not None:
            logger.debug(
                f"Loading whisper model from model_dir {
                    model_dir}. modelsize parameter is not used."
            )
            model_size_or_path = model_dir
        elif modelsize is not None:
            model_size_or_path = self.translate_model_name(modelsize)
            logger.debug(
                f"Loading whisper model {modelsize}. You use mlx whisper, so {
                    model_size_or_path} will be used."
            )
    ...
    def translate_model_name(self, model_name):
        ...
        if mlx_model_path:
            ...
            raise ValueError(
                f"Model name '{
                    model_name}' is not recognized or not supported."
            )
...
class OpenaiApiASR(ASRBase):
    ...
    def transcribe(self, audio_data, prompt=None, *args, **kwargs):
        ...
        logger.debug(
            f"OpenAI API processed accumulated {
